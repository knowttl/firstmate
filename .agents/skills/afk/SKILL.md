...
- `stale` with a terminal status or bare legacy captain-relevant line -> escalate.
  ...
  If the pane is still idle past `FM_STALE_ESCALATE_SECS` (default 240s), housekeeping re-reads the crew's current state and escalates it as a possible wedge unless the crew is still provably working - a long in-contract pipeline call keeps a healthy pane frozen for far longer than that threshold.
  A crew that keeps verifying as working is deferred one threshold window at a time, up to `FM_WEDGE_WORKING_ESCALATE_SECS`; [`docs/architecture.md`](../../../docs/architecture.md) owns that contract.
  This bounds wedge-detection latency to one threshold window past the crew going quiet, and to the allowance for a frozen pipeline that keeps reporting itself as running: a delay, never a loss.
