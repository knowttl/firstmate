...
3. **In explicit file mode only, compose and replace the detailed report file.**
   ...
   For a richer review surface, optionally offer an Atelier board with `atelier-axi` when the report has enough structure to deserve one, but only after the required digest is ready.
