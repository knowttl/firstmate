...
To pin ONE secondmate differently from the fleet, write `config/secondmate-harness.<id>`, the per-id override read before this file for that id with the same format, precedence, and durability ([`docs/configuration.md`](../../../docs/configuration.md#harness-support) owns its schema).
