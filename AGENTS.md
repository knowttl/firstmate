...
A cycle is down only when its background task completes carrying a WAKE REASON (`signal`/`stale`/`parked`/`check`/`heartbeat`): that is the watcher firing, and that is the one moment to handle the wake and then start exactly one fresh cycle.
...
After arming it, do not send idle progress updates to the captain; wait until it returns `signal`, `stale`, `parked`, `check`, or `heartbeat`, unless the captain asks for status.
...
bin/fm-watch.sh            # the watcher itself; exits with: signal|stale|parked|check|heartbeat
...
5. `check:` a per-task poll fired (usually a merge, or X mode when enabled); act on it.
