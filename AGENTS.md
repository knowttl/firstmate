...
config/secondmate-harness.<id>  optional per-secondmate override of config/secondmate-harness for that one id, same format and precedence (docs/configuration.md); LOCAL, gitignored; absent = the fleet-wide file, unchanged. Also the primary's own setting; NOT inherited
...
state/               runtime records and signals; gitignored
  ...
  .watch.lock .wake-queue.lock .wake-annotation.lock watcher singleton, queue serialization, and drain annotation serialization locks
  .claude-autoarm.lock .claude-autoarm-epoch .claude-autoarm-failure-notified .claude-autoarm-failure-alarmed .turnend-claude-blocks .turnend-claude-blocks.lock   Claude Stop auto-arm single-flight, epoch, failure-episode, attended-alarm, guard-budget, and budget-lock records; never touch
  .hash-* .count-* .stale-* .stale-since-* .paused-* .wedge-escalations-* .wedge-verified-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
  .drain-cursor-<id> .drain-retry-{direct,historical}-<id>  per-status-file drain read cursor and durable capped-output retry; drain internal, never touch
...
Use `gh-axi` for GitHub, `chrome-devtools-axi` for browser work, and `atelier-axi` for structured decisions or reports; consult current help rather than memorizing flags.
...
Use plain chat for a yes-or-no decision and `atelier-axi` only when several options or a structured report benefit from a visual surface.
