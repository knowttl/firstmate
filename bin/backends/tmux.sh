...
#
# fm-spawn.sh acquires a durable treehouse lease outside the pane, then sends an
# explicit `cd` to the leased path through this adapter. The adapter remains
# responsible only for the session-provider operations.
#
...
# (the leased-path `cd`, the GOTMPDIR export) that already run this exact sequence
