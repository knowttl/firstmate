#!/usr/bin/env bash
# fm-bearings-board.sh - build and arm the /bearings atelier fleet board.
#
# The board is the captain-facing interactive surface of /bearings atelier: the
...
#            template at the stable board path. Establish or resume the Atelier
...
#            `board: <path>`, then includes atelier-axi's session output and
...
#
# The board path is stable - $FM_HOME/.atelier/bearings-board.html - so a
# re-invocation rebuilds the same file in place, which keeps the same Atelier
...
board_path() { printf '%s/.atelier/bearings-board.html\n' "$FM_HOME"; }
...
command_build() {
  ...
  command -v atelier-axi >/dev/null 2>&1 || fail "atelier-axi is not installed"
  atelier-axi "$board" || fail "cannot establish the board Atelier session"
  printf 'served: %s\n' "$board"

  sid=$("$SCRIPT_DIR/fm-procevent-atelier.sh" source-id "$board") \
  ...
  if "$SCRIPT_DIR/fm-procevent.sh" list | awk 'NR > 1 { print $1 }' | grep -Fxq "$sid"; then
    ...
    "$SCRIPT_DIR/fm-procevent-atelier.sh" arm "$board" >/dev/null \
