...
# Atelier, so every rule about what a keyed answer DOES belongs to the one intake in
# bin/fm-decision-hold.sh, which the runner feeds. An Atelier review is just an
...
cmd_arm() {
  ...
  "$SCRIPT_DIR/fm-procevent.sh" register atelier "$id" -- atelier-axi poll "$real" || exit 1
  printf 'armed: %s\n' "$id"
  printf 'artifact: %s\n' "$real"
}
