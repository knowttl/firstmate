#!/usr/bin/env bash
# Atelier adapter for the generic process-to-event runner.
#
# Usage:
#   fm-procevent-atelier.sh arm <artifact.html>
#   fm-procevent-atelier.sh classify <result-file>
#   fm-procevent-atelier.sh terminal <result-file>
#   fm-procevent-atelier.sh silent <result-file>
#   fm-procevent-atelier.sh answers <result-file>
#   fm-procevent-atelier.sh read <result-file>
#   fm-procevent-atelier.sh source-id <artifact.html>
#   fm-procevent-atelier.sh retire <artifact.html>
#   fm-procevent-atelier.sh poll <artifact.html>
#
...
#            what Atelier delivered. The session-ending freeform message
...
# terminal   Exit 0 when the captured result means this Atelier source will never
#            produce another result, so the runner may retire it; any other exit
#            keeps it armed. This is the generic adapter contract bin/fm-procevent.sh
#            calls, and the only place Atelier's notion of "ended" is decided.
...
#            only place Atelier's notion of "nothing was said" is decided.
#
# AN EMPTY BOARD CLOSE IS NOT NEWS, and that is what `silent` exists to say.
# Closing a review surface that carried nothing is the single most common Atelier
...
#
# This adapter is deliberately thin. It owns only what is specific to Atelier:
...
# to Atelier, so every rule about what a keyed answer DOES belongs to the one
# intake in bin/fm-captain-hold.sh, which the runner feeds. An Atelier review is
...
#
# It wraps ONLY the currently published interface, verified against 0.3.3:
#   Usage: atelier-axi poll <html-file> [--agent-reply "..."] [--full]
# The additive --full flag returns the complete DOM snapshot; this adapter does
# not pass it, so results arrive with the default truncated snapshot.
...
# spent are all printed straight through and captured normally. The retry is an
# Atelier fact, so the generic runner in bin/fm-procevent.sh stays
...
# Canonical identity is physical, not the path string: Atelier itself keys a
...
cmd_arm() {
  ...
  [ -n "$artifact" ] || usage
  [ "$#" -eq 1 ] || usage
  command -v atelier-axi >/dev/null 2>&1 || die "atelier-axi is not installed"
  ...
  "$SCRIPT_DIR/fm-procevent.sh" register atelier "$id" \
    -- "$SCRIPT_DIR/fm-procevent-atelier.sh" poll "$real" || exit 1
  printf 'armed: %s\n' "$id"
  printf 'artifact: %s\n' "$real"
}
...
poll_response_filter() {  # <response-file>
  perl -e '
    ...
    use warnings;
    my ($stage) = @ARGV;
    my $expected = "error: Atelier Editor poll response was interrupted\ncode: SERVER_ERROR\n";
...
# Seconds between retries. FM_ATELIER_POLL_RETRY_DELAY is a bounded test
...
# meaning anything.
poll_retry_delay() {
  local delay=${FM_ATELIER_POLL_RETRY_DELAY-}
  if [ -z "$delay" ]; then
    printf '%s\n' "$POLL_RETRY_DELAY_DEFAULT"
    return 0
  fi
  case "$delay" in
    *[!0-9]*) die "FM_ATELIER_POLL_RETRY_DELAY must be whole seconds from 0 to $POLL_RETRY_DELAY_MAX: $delay" ;;
  esac
  [ "$delay" -le "$POLL_RETRY_DELAY_MAX" ] \
    || die "FM_ATELIER_POLL_RETRY_DELAY must be whole seconds from 0 to $POLL_RETRY_DELAY_MAX: $delay"
  printf '%s\n' "$delay"
}
...
cmd_poll() {
  ...
  response=$(mktemp "${TMPDIR:-/tmp}/fm-atelier-poll.XXXXXX") || die "cannot stage the poll response"
  ...
  while :; do
    atelier-axi poll "$artifact" | poll_response_filter "$response"
...
cmd_classify() {
  ...
  error_code=$(awk '
    ...
    in_error { exit }
  ' "$file")
  if [ "$error_code" = NOT_FOUND ] || [[ "$error_message" == "No active Atelier Editor session"* ]]; then
    printf 'missing\n'
  else
    printf 'unknown\n'
  fi
...
# retirement. Atelier's notion of "ended" lives here and nowhere else: an ended
...
# announcing, for the generic runner's silence seam. Atelier's notion of "nothing
