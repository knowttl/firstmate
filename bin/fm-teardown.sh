...
cleanup_firstmate_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    rm -f "$sub_state/$child_id.status" "$sub_state/$child_id.turn-ended" \
      ...
      "$sub_state/.drain-cursor-$child_id" "$sub_state/.drain-retry-direct-$child_id" \
      "$sub_state/.drain-retry-historical-$child_id"
  done
}
...
rm -f "$STATE/$ID.status" "$STATE/$ID.turn-ended" "$STATE/$ID.meta" \
  ...
  "$STATE/.$ID.open-decisions-cursor" "$STATE/.drain-cursor-$ID" \
  "$STATE/.drain-retry-direct-$ID" "$STATE/.drain-retry-historical-$ID"
