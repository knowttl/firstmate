...
#                          tool process. That wedge timer is exempt from the
#                          working deferral above: the busy signal a
#                          re-verification would read is the very one
#                          BUSY_TURN_MAX_SECS stopped trusting.
...
# provably-working stale that stops re-verifying or reaches its working deferral
# allowance, or anything unknown) is written to the durable queue and exits. That
# wakes the LLM through the background-task completion. The same classifier
...
#
# At the threshold, two independent deferrals run before escalating. First,
# unless the caller passed `nodefer`, wedge_escalation_deferred re-reads the crew
# state, whose bounded deferral policy is documented in docs/architecture.md; this
# function owns the per-window .wedge-verified marker, so that crew state is
# re-read at most once per STALE_ESCALATE_SECS per pane rather than every poll.
# Then crew_worktree_written_since probes the task's own worktree; that bounded
# walk also runs at most once per window per STALE_ESCALATE_SECS, never per poll.
# `nodefer` suppresses ONLY the first deferral, for a caller whose bound it would
# defeat rather than refine: the busy-turn path is already past BUSY_TURN_MAX_SECS
# precisely because a busy pane's liveness signal is no longer trusted, and that
# same busy pane is exactly what crew_is_provably_working reads, so re-verifying it
# would only re-affirm the discredited signal. The worktree write probe reads the
# filesystem instead, independent of that signal, so it still runs on that path.
wedge_timer_check() {  # <window> <since-file> <triage-label> <escalation-count-file> <task> [nodefer]
  local win=$1 since_file=$2 label=$3 escalation_file=$4 task=$5 defer=${6:-defer} since age n reason vf
  ...
  case "$since" in
    ...
    *)
      ...
      if [ "$age" -ge "$STALE_ESCALATE_SECS" ]; then
        ...
        if [ "$defer" != nodefer ]; then
          wedge_escalation_deferred "$task" "$age" \
...
busy_turn_bound_check() {  # <window> <task> <hash> <since-file> <escalation-file>
  ...
  wedge_timer_check "$win" "$since_file" "busy (no completed turn)" "$escalation_file" "$task" nodefer
  return 1
}
