...
1. **Resolve the profile.**
   ...
   Otherwise a `kind=secondmate` task re-resolves its durable `config/secondmate-harness` pin, including that file's optional model and effort tokens and the per-secondmate override `config/secondmate-harness.<id>` that precedes it ([`docs/configuration.md`](configuration.md#harness-support)), exactly as every other respawn does - so setting the pin and relaunching is the ordinary way to move a secondmate's runtime.
