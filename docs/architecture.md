...
Escalating restarts the idle clock, so a pane that keeps reporting itself as working repeats at roughly the allowance while one that has stopped verifying returns to the plain threshold cadence; either way the escalation count keeps accumulating.
