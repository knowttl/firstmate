...
- `git` with GitHub auth, `no-mistakes`, `gh-axi`, `chrome-devtools-axi`, and the review tool (default `lavish-axi`, overridable via `config/review-tool`) - the same universal requirements as tmux, minus `tmux` and `treehouse` (Orca replaces both).
