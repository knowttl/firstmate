...
On Linux the worker identifies its own processes by kernel start ticks, so a host clock step from NTP, VM or WSL2 time sync, or resume never makes a healthy worker look like a stranger and never starts a second supervisor beside it; `bin/fm-remote-job-lib.sh` owns that identity contract.
