...
A close from a watcher that has successfully refreshed its own beacon and whose beacon remains fresh at the final release boundary, with none of those, opens no episode, because a provably healthy home with nothing to re-present would otherwise demand an acknowledgement turn for an empty queue on every ordinary one-shot close and re-announce itself every cycle.
