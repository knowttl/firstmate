...
Every durable queue append publishes downtime, and so does every watcher close that leaves work for the next arm to resurface: queued durable wakes, an open captain decision, or an episode that is already open.
An idle close is permitted only when the durable queue is empty, every status log can be classified with no open decision, and the watcher has successfully refreshed its own beacon.
The reuse-only publication evaluates that complete predicate inside the marker-lock critical section and skips a new episode only on affirmative current proof; any queued wake, open decision, indeterminate scan, or failed health proof falls back to ordinary downtime publication.
A quiet healthy home therefore opens no new episode on an ordinary close, so `state/.watcher-down` converges to cleared - it stays absent or settled at `acked:<generation>` rather than re-announcing supervision-down with a fresh generation every turn.
An accepted residual remains: a needs-decision line appended through a crewmate's unlocked direct status-log write while the strict scan is folding can be missed by that one close.
The consequence is bounded to a one-cycle delay rather than a lost decision, because the append changes the status file's signature so the next watcher's ordinary signal scan wakes on it, and every drain prints its OPEN DECISIONS section from its own independent fold regardless of the recovery marker.
...
It also drives both directions of the close-time boundary against the real watcher and drain: a healthy home with nothing queued must converge to no episode and demand no acknowledgement across repeated cycles, while a close leaving a queued wake or an unchanged open decision must still open an episode the next arm recovers.
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows, a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before proving its terminated stale-beacon episode is published and resurfaced, termination before a new lock owner's first successful beat despite an inherited fresh beacon, a beacon that expires during cleanup or while waiting for the marker lock, a decision appended during that marker-lock wait, and an unclassifiable status log.
