...
A close from a watcher whose own beacon is fresh, with none of those, opens no episode, because a provably healthy home with nothing to re-present would otherwise demand an acknowledgement turn for an empty queue on every ordinary one-shot close and re-announce itself every cycle.
...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows, and a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before proving its terminated stale-beacon episode is published and resurfaced.
