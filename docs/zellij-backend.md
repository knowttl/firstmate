...
- The same universal requirements as tmux (a verified crew harness, git with GitHub auth, node, treehouse, no-mistakes, gh-axi, chrome-devtools-axi, and the review tool - default lavish-axi, overridable via `config/review-tool`); treehouse still provides the worktree, zellij only provides the session.
