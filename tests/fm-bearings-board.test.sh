...
test_path_is_stable_and_home_scoped() {
  local home
  home=$(make_home path)
  [ "$(run_board "$home" path)" = "$home/.atelier/bearings-board.html" ] \
...
test_build_refuses_malformed_payloads_before_touching_the_board() {
  ...
  board="$home/.atelier/bearings-board.html"
...
test_build_injects_binds_then_arms() {
  ...
  assert_contains "$out" "served: $board" "build did not establish the Atelier session: $out"
  ...
  sid=$(run_atelier_source_id "$home" "$board")
...
EOF
  ...
  cat > "$runtime/bin/fm-procevent-atelier.sh" <<'SH'
#!/usr/bin/env bash
set -eu
if [ "${1:-}" = arm ]; then
  artifact=${2:-}
  "$REAL_ATELIER_ADAPTER" arm "$artifact" >/dev/null
  sid=$("$REAL_ATELIER_ADAPTER" source-id "$artifact")
  "$REAL_PROCEVENT" start "$sid" >/dev/null
  exit 0
fi
exec "$REAL_ATELIER_ADAPTER" "$@"
SH
  chmod +x "$runtime/bin/fm-procevent-atelier.sh"
  cat > "$home/fakebin/atelier-axi" <<'SH'
#!/usr/bin/env bash
if [ "${1:-}" != poll ]; then
  exit 0
fi
...
EOF
SH
  chmod +x "$home/fakebin/atelier-axi"
  ...
  PATH="$home/fakebin:$PATH" FM_ROOT_OVERRIDE="$runtime" FM_HOME="$home" \
    ...
    REAL_ATELIER_ADAPTER="$ROOT/bin/fm-procevent-atelier.sh" \
...
SH
  ...
  rc=$?
  set -e
  [ "$rc" -ne 0 ] || fail "build continued after Atelier session establishment failed"
  sid=$(run_atelier_source_id "$home" "$home/.atelier/bearings-board.html")
  ! run_decisions "$home" binding "$sid" >/dev/null 2>&1 \
    || fail "build bound the board before its Atelier session existed"
  ! run_procevent "$home" list | awk 'NR > 1 { print $1 }' | grep -Fxq "$sid" \
    || fail "build armed the board before its Atelier session existed"
  pass "build establishes the Atelier session before binding and arming"
}

run_atelier_source_id() {  # <home> <artifact>
  local home=$1
  PATH="$home/fakebin:$PATH" FM_HOME="$home" \
    FM_STATE_OVERRIDE="$home/state" FM_DATA_OVERRIDE="$home/data" \
    FM_PROCEVENT_CLAIM_ROOT="$home/procevent-claims" \
    "$ROOT/bin/fm-procevent-atelier.sh" source-id "$2"
}
...
test_build_refuses_a_template_without_exactly_one_slot() {
  ...
  assert_absent "$home/.atelier/bearings-board.html" "a refused template still produced a board"
...
test_charted_kind_is_optional_and_accepts_both_values() {
  ...
  extract_payload "$home/.atelier/bearings-board.html" | jq -e '
