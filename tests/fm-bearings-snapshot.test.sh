...
# The Atelier-103 defect, end to end: a COMPLETED scout that raised a decision and
...
test_completed_scout_report_not_pending() {
  ...
  fakebin=$(make_fakebin "$home")
  mkdir -p "$home/projects/lav-wt" "$home/data/atelier-103"
  ...
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.decisions_open | any(.[]; .id == "atelier-103") | not)
      and (.reports | any(.[]; .id == "atelier-103"))
