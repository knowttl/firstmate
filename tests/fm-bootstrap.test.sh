...
# 'MISSING: gh-axi (install: ...)', 'MISSING: atelier-axi (install: ...)', and
...
# which atelier-axi version is on PATH.
...
make_fake_toolchain() {
  ...
  fm_fake_version_tool "$fakebin" atelier-axi FM_FAKE_ATELIER_AXI_VERSION 0.3.3
...
test_atelier_axi_min_version() {
  local label version mode case_dir fakebin out missing n
  missing='MISSING: atelier-axi (install: npm install -g atelier-axi && atelier-axi setup hooks)'
  n=0
  while IFS='^' read -r label version mode; do
    [ -n "$label" ] || continue
    n=$((n + 1))
    case_dir="$TMP_ROOT/atelier-axi-$n"
    mkdir -p "$case_dir/home/config"
    printf '%s\n' manual > "$case_dir/home/config/backlog-backend"
    fakebin=$(make_fake_toolchain "$case_dir")
    out=$(PATH="$fakebin:$BASE_PATH" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
      FM_FAKE_TREEHOUSE_LEASE_HELP=1 FM_FAKE_ATELIER_AXI_VERSION="$version" "$ROOT/bin/fm-bootstrap.sh")
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      missing)
...
minimum atelier-axi version is accepted^0.3.3^empty
newer atelier-axi patch is accepted^0.3.4^empty
newer atelier-axi minor is accepted^0.4.0^empty
newer atelier-axi major is accepted^1.0.0^empty
the patch just below the floor reports an upgrade^0.3.2^missing
much older atelier-axi minor reports an upgrade^0.0.9^missing
unparseable atelier-axi version reports an upgrade^atelier-axi development build^missing
ROWS
  pass "bootstrap enforces atelier-axi minimum version"
}
