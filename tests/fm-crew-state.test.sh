...
# its frame still shows. The semantic busy contract classifies from the crew's
# own record and does not read endpoint liveness on this path, so this
# presence gate remains the thing that settles an agent-less endpoint here.
