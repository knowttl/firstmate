...
# The Atelier review adapter, run against this suite's isolated home. The
...
run_atelier() {  # <home> <command args...>
  local home=$1
  shift
  PATH="$home/fakebin:$PATH" FM_ROOT_OVERRIDE="$ROOT" FM_HOME="$home" \
    FM_STATE_OVERRIDE="$home/state" FM_DATA_OVERRIDE="$home/data" \
    FM_PROCEVENT_CLAIM_ROOT="$home/procevent-claims" \
    "$ROOT/bin/fm-procevent-atelier.sh" "$@"
}
...
# end through the channel that actually carried it. A Atelier review deck exposes
...
test_bound_channel_answers_close_their_holds_at_answer_time() {
  ...
  # captain will answer. atelier-axi is stubbed: nothing here starts a real server.
  artifact="$home/data/$id/review.html"
  printf '<h1>Sample eval proposal</h1>\n' > "$artifact"
  fm_fake_exit0 "$home/fakebin" atelier-axi
  sid=$(run_atelier "$home" source-id "$artifact") || fail "could not derive the review source id"
  # Binding a source to its decision origin is the GENERAL capability, not a
  # Atelier feature: it is recorded through the same owner that closes the holds,
  ...
  run_atelier "$home" arm "$artifact" >/dev/null || fail "could not arm the review deck"
...
EOF
  ...
  out=$(run_atelier "$home" answers "$result") || fail "could not read the captured answers"
  ...
  # FIXTURE adapter that is not Atelier at all and knows nothing about holds - it
  # only prints keyed answers - so what is proven is that ANY bound channel with
  # an `answers` command gets closure, not that Atelier is wired specially.
...
case "\${1-}" in
  answers) exec "$ROOT/bin/fm-procevent-atelier.sh" answers "\${2-}" ;;
esac
exit 2
SH
  ...
  set +e
  out=$(run_atelier "$home" answers "$result" \
...
test_unbound_source_closes_no_hold() {
  ...
  home=$(make_home atelier-unbound)
  ...
  fm_fake_exit0 "$home/fakebin" atelier-axi
  sid=$(run_atelier "$home" source-id "$artifact") || fail "could not derive the unbound source id"
  run_atelier "$home" arm "$artifact" >/dev/null || fail "could not arm the unbound review"
