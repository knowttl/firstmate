...
expect_failure "prior runner remains active" env FM_HOME="$H_ACTIVE_RUNNER" "$PROCEVENT" register atelier active-source -- /bin/echo built-in
...
FM_HOME="$H_ACTIVE_RUNNER" "$PROCEVENT" register atelier active-source -- /bin/echo built-in >/dev/null
FM_HOME="$H_ACTIVE_RUNNER" "$PROCEVENT" retire active-source --if-matches atelier -- /bin/echo built-in >/dev/null
...
for control_kind in tab newline; do
  ...
  FM_HOME="$H_STATE_OVERRIDE" FM_STATE_OVERRIDE="$control_state" \
    "$PROCEVENT" register atelier "$control_source" -- /bin/echo control >/dev/null
...
FM_HOME="$H_LEGACY_LINK" FM_STATE_OVERRIDE="$LEGACY_LINK_STATE" \
  "$PROCEVENT" register atelier legacy-link-source -- /bin/echo legacy-link >/dev/null
FM_HOME="$H_LEGACY_LINK" FM_STATE_OVERRIDE="$LEGACY_LINK_STATE" \
...
FM_HOME="$H_LEGACY" "$PROCEVENT" register atelier legacy-source -- /bin/echo legacy >/dev/null
expect_failure "does not match the expected owner" env FM_HOME="$H_LEGACY" "$PROCEVENT" retire legacy-source --if-matches atelier -- /bin/echo replacement
assert_present "$H_LEGACY/state/procevent/legacy-source.source" "legacy conditional mismatch retired the registration"
FM_HOME="$H_LEGACY" "$PROCEVENT" retire legacy-source --if-matches atelier -- /bin/echo legacy >/dev/null
...
for remote_file in \
  fm-extension.mjs fm-extension-launch-barrier.mjs fm-extension.sh fm-procevent.sh fm-procevent-lib.sh fm-procevent-extension-capture.pl fm-procevent-atelier.sh \
...
expect_failure "prior runner remains active" remote_direct fm-procevent.sh register atelier remote-active-source -- /bin/echo remote-built-in
...
remote_direct fm-procevent.sh register atelier remote-active-source -- /bin/echo remote-built-in >/dev/null
remote_direct fm-procevent.sh retire remote-active-source --if-matches atelier -- /bin/echo remote-built-in >/dev/null
