...
# body and the stale status line contain decision-like prose. This is the Atelier-103
...
test_completed_scout_report_is_pointer_not_pending() {
  ...
  mkdir -p "$home/projects/scout-wt" "$home/data/atelier-103"
  fm_write_meta "$home/state/atelier-103.meta" \
    "window=firstmate:fm-atelier-103" \
    "worktree=$home/projects/scout-wt" \
    "project=firstmate" \
    "harness=claude" \
    "kind=scout" \
    "mode=scout"
  record_claude_idle "$home/state" atelier-103
  # Stale needs-decision, then the scout finished (done). No keyed resolution.
  printf 'needs-decision: adopt approach A or B for Atelier issue 103\n' > "$home/state/atelier-103.status"
  printf 'done: report ready at data/atelier-103/report.md\n' >> "$home/state/atelier-103.status"
  # Completed report whose PROSE reads like the decision.
  printf '# Atelier 103\nThe open question is whether to adopt approach A or B.\nThis needs a captain decision. Recommendation: A.\n' > "$home/data/atelier-103/report.md"
  fakebin=$(make_fakebin "$home")
  out=$(PATH="$fakebin:$PATH" FM_HOME="$home" "$SNAPSHOT" --json)
  printf '%s' "$out" | jq -e '
    .tasks[] | select(.id == "atelier-103")
