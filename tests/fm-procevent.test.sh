#!/usr/bin/env bash
# Behavior tests for the generic process-to-event runner and its Atelier adapter.
#
...
# depends on a discovery timer. The Atelier adapter is exercised through its own
# public commands against the currently published poll shape; no live Atelier
# server is started.
#
# Delivery is deliberately NOT asserted as at-least-once or lossless: the
# published Atelier poll clears feedback destructively before returning it, so
...
out=$(pe_register "$H1" atelier src-one -- "$BLOCKER" "$TRIG" "payload one")
...
assert_contains "$payload" "procevent atelier src-one 1" "completion publishes the committed result sequence"
...
assert_grep 'atelier' "${RESULT%.result}.adapter" "the captured result retains its immutable adapter"
...
pe_register "$HSYM" atelier symlinked-src -- "$BLOCKER" "$SYM_TRIGGER" "symlinked payload" >/dev/null
...
assert_contains "$(wake_payloads "$HSYM")" "procevent atelier symlinked-src 1" \
...
pe_register "$HPG" atelier direct-src -- "$BLOCKER" "$DIRECT_TRIGGER" "direct result" >/dev/null
...
exit 0;
PL
pe_register "$HPG" atelier shared-src -- "$BLOCKER" "$SHARED_TRIGGER" "shared result" >/dev/null
...
assert_contains "$(wake_payloads "$H2")" "procevent atelier src-cut 7" "durable adapter identity survives without a registration"
...
assert_contains "$(wake_payloads "$H2")" "procevent atelier src-cut 7" "the repeat wake preserves its deduplication identity"
...
# The dogfood defect: a real armed Atelier source received one human `Send & End`
...
ATELIER_BIN=$(fm_fakebin "$TMP_ROOT/atelier-stub")
ATELIER_POLL_COUNT="$TMP_ROOT/atelier-poll-count"
export ATELIER_POLL_COUNT
cat > "$ATELIER_BIN/atelier-axi" <<'SH'
#!/usr/bin/env bash
# Stand-in for `atelier-axi poll <file>` around a human `Send & End`: the final
# feedback is delivered exactly once carrying session_ended, and every later
# poll returns an empty ended session immediately.
n=$(cat "$ATELIER_POLL_COUNT" 2>/dev/null || echo 0)
n=$((n + 1))
printf '%s\n' "$n" > "$ATELIER_POLL_COUNT"
if [ "$n" = 1 ]; then
  printf 'session:\n  file: /review.html\n  status: feedback\n  session_ended: true\n  ended_by: user\nfeedback[1]{text}:\n  ship it\n'
else
  printf 'session:\n  file: /review.html\n  status: ended\n  ended_by: user\n'
...
SH
chmod +x "$ATELIER_BIN/atelier-axi"
REVIEW_ART="$TMP_ROOT/review.html"
printf '<h1>review</h1>\n' > "$REVIEW_ART"
atelier_id=$("$ROOT/bin/fm-procevent-atelier.sh" source-id "$REVIEW_ART")
PE_TRACKED+=("$HLT|$atelier_id")
PATH="$ATELIER_BIN:$PATH" FM_HOME="$HLT" "$ROOT/bin/fm-procevent-atelier.sh" arm "$REVIEW_ART" >/dev/null
for _ in $(seq 1 6); do
  PATH="$ATELIER_BIN:$PATH" pe "$HLT" reconcile >/dev/null
  sleep 0.3
done
[ "$(cat "$ATELIER_POLL_COUNT")" = 1 ] \
  || fail "an ended review kept being polled: $(cat "$ATELIER_POLL_COUNT") polls for one Send & End"
[ "$(count_results "$HLT" "$atelier_id")" = 1 ] \
  || fail "one Send & End produced $(count_results "$HLT" "$atelier_id") captured results"
[ "$(wake_payloads "$HLT" | sort -u | grep -c .)" = 1 ] \
  || fail "one Send & End produced more than one distinct event: $(wake_payloads "$HLT" | sort -u)"
assert_contains "$(wake_payloads "$HLT")" "procevent atelier $atelier_id 1" "the human's final feedback is announced"
assert_absent "$HLT/state/procevent/$atelier_id.source" "the ended review source retires automatically"
assert_absent "$FM_PROCEVENT_CLAIM_ROOT/$atelier_id.claim" "the ended review releases its owned claim"
ATELIER_RESULT=$(first_result "$HLT" "$atelier_id" || true)
assert_grep 'ship it' "$ATELIER_RESULT" "automatic retirement retains the human's final feedback"
out=$(PATH="$ATELIER_BIN:$PATH" FM_HOME="$HLT" "$ROOT/bin/fm-procevent-atelier.sh" retire "$REVIEW_ART")
assert_contains "$out" "retired: $atelier_id" "explicit adapter retirement stays supported after automatic retirement"
...
EMPTY_BIN=$(fm_fakebin "$TMP_ROOT/atelier-empty-stub")
cat > "$EMPTY_BIN/atelier-axi" <<'SH'
#!/usr/bin/env bash
# Stand-in for `atelier-axi poll <file>` when the captain closes a board he said
# nothing on: an ended session carrying no queued content at all.
printf 'session:\n  file: /quiet.html\n  status: ended\n  ended_by: user\n'
SH
chmod +x "$EMPTY_BIN/atelier-axi"
QUIET_ART="$TMP_ROOT/quiet-board.html"
printf '<h1>quiet</h1>\n' > "$QUIET_ART"
quiet_id=$("$ROOT/bin/fm-procevent-atelier.sh" source-id "$QUIET_ART")
PE_TRACKED+=("$HEMPTY|$quiet_id")
PATH="$EMPTY_BIN:$PATH" FM_HOME="$HEMPTY" \
  "$ROOT/bin/fm-procevent-atelier.sh" arm "$QUIET_ART" >/dev/null
...
ANSWER_BIN=$(fm_fakebin "$TMP_ROOT/atelier-answer-stub")
cat > "$ANSWER_BIN/atelier-axi" <<'SH'
#!/usr/bin/env bash
# Stand-in for `atelier-axi poll <file>` on a real `Send & End`: the captain's
...
SH
chmod +x "$ANSWER_BIN/atelier-axi"
ANSWER_ART="$TMP_ROOT/answered-board.html"
printf '<h1>answered</h1>\n' > "$ANSWER_ART"
answer_id=$("$ROOT/bin/fm-procevent-atelier.sh" source-id "$ANSWER_ART")
PE_TRACKED+=("$HANSWER|$answer_id")
PATH="$ANSWER_BIN:$PATH" FM_HOME="$HANSWER" \
  "$ROOT/bin/fm-procevent-atelier.sh" arm "$ANSWER_ART" >/dev/null
...
assert_contains "$(wake_payloads "$HANSWER")" "procevent atelier $answer_id 1" \
...
#     error: Atelier Editor poll response was interrupted
#     code: SERVER_ERROR
...
ATELIER_SCRIPTED_BIN=$(fm_fakebin "$TMP_ROOT/atelier-scripted-stub")
cat > "$ATELIER_SCRIPTED_BIN/atelier-axi" <<'SH'
#!/usr/bin/env bash
# Stand-in for `atelier-axi poll <file>`, scripted per scenario: ATELIER_SCRIPT
...
n=$(cat "$ATELIER_COUNT" 2>/dev/null || echo 0)
n=$((n + 1))
printf '%s\n' "$n" > "$ATELIER_COUNT"
read -r -a plan <<< "$ATELIER_SCRIPT"
i=$((n - 1))
[ "$i" -ge "${#plan[@]}" ] && i=$((${#plan[@]} - 1))
case "${plan[$i]}" in
  interrupt)
    printf 'error: Atelier Editor poll response was interrupted\ncode: SERVER_ERROR\n'; exit 1 ;;
  near-interrupt)
    printf 'error: Atelier Editor poll response was interrupted \ncode: SERVER_ERROR\n'; exit 1 ;;
  other-server-error)
    printf 'error: Atelier Editor session store is unavailable\ncode: SERVER_ERROR\n'; exit 1 ;;
  feedback)
    printf 'session:\n  file: /board.html\n  status: feedback\n  session_ended: true\n  ended_by: user\nfeedback[1]{text}:\n  ship it\n' ;;
  stream)
    printf 'x%.0s' {1..4096}
    printf 'ready\n' > "$ATELIER_STREAM_READY"
    while [ ! -e "$ATELIER_STREAM_RELEASE" ]; do sleep 0.05; done
    printf '\n' ;;
esac
SH
chmod +x "$ATELIER_SCRIPTED_BIN/atelier-axi"
export ATELIER_COUNT ATELIER_SCRIPT
...
export FM_ATELIER_POLL_RETRY_DELAY=0
...
printf '<h1>retry</h1>\n' > "$RETRY_ART"
retry_id=$("$ROOT/bin/fm-procevent-atelier.sh" source-id "$RETRY_ART")
PE_TRACKED+=("$HRETRY|$retry_id")
ATELIER_COUNT="$TMP_ROOT/retry-count"; ATELIER_SCRIPT="interrupt interrupt feedback"
PATH="$ATELIER_SCRIPTED_BIN:$PATH" FM_HOME="$HRETRY" \
  "$ROOT/bin/fm-procevent-atelier.sh" arm "$RETRY_ART" >/dev/null
PATH="$ATELIER_SCRIPTED_BIN:$PATH" pe "$HRETRY" reconcile >/dev/null
wait_for "$HRETRY/state/.wake-queue" || fail "feedback after interrupted polls produced no wake"
[ "$(cat "$ATELIER_COUNT")" = 3 ] \
  || fail "the interrupted listener was polled $(cat "$ATELIER_COUNT") times, not the two quiet retries plus the delivering poll"
...
assert_contains "$(wake_payloads "$HRETRY")" "procevent atelier $retry_id 1" \
...
pass "a transient Atelier poll interruption is retried quietly and never announced"
...
exh_id=$("$ROOT/bin/fm-procevent-atelier.sh" source-id "$EXH_ART")
PE_TRACKED+=("$HEXH|$exh_id")
ATELIER_COUNT="$TMP_ROOT/exhaust-count"; ATELIER_SCRIPT="interrupt"
PATH="$ATELIER_SCRIPTED_BIN:$PATH" FM_HOME="$HEXH" \
  "$ROOT/bin/fm-procevent-atelier.sh" arm "$EXH_ART" >/dev/null
PATH="$ATELIER_SCRIPTED_BIN:$PATH" pe "$HEXH" start "$exh_id" >/dev/null
[ "$(cat "$ATELIER_COUNT")" = 13 ] \
  || fail "the retry bound polled $(cat "$ATELIER_COUNT") times, not the first poll plus 12 bounded retries"
[ "$(count_results "$HEXH" "$exh_id")" = 1 ] \
  || fail "exhaustion produced $(count_results "$HEXH" "$exh_id") captured results instead of one"
assert_contains "$(wake_payloads "$HEXH")" "procevent atelier $exh_id 1" \
...
PATH="$ATELIER_SCRIPTED_BIN:$PATH" FM_HOME="$HEXH" \
  "$ROOT/bin/fm-procevent-atelier.sh" retire "$EXH_ART" >/dev/null
...
printf '<h1>other</h1>\n' > "$OTHER_ART"
other_id=$("$ROOT/bin/fm-procevent-atelier.sh" source-id "$OTHER_ART")
PE_TRACKED+=("$HOTHER|$other_id")
ATELIER_COUNT="$TMP_ROOT/other-count"; ATELIER_SCRIPT="other-server-error"
PATH="$ATELIER_SCRIPTED_BIN:$PATH" FM_HOME="$HOTHER" \
  "$ROOT/bin/fm-procevent-atelier.sh" arm "$OTHER_ART" >/dev/null
PATH="$ATELIER_SCRIPTED_BIN:$PATH" pe "$HOTHER" start "$other_id" >/dev/null
[ "$(cat "$ATELIER_COUNT")" = 1 ] \
  || fail "an unrelated SERVER_ERROR was retried $(cat "$ATELIER_COUNT") times instead of surfacing at once"
assert_contains "$(wake_payloads "$HOTHER")" "procevent atelier $other_id 1" \
  "an unrelated SERVER_ERROR is captured and announced immediately"
PATH="$ATELIER_SCRIPTED_BIN:$PATH" FM_HOME="$HOTHER" \
  "$ROOT/bin/fm-procevent-atelier.sh" retire "$OTHER_ART" >/dev/null
pass "only the exact interruption is retried; an unrelated SERVER_ERROR still surfaces"
unset FM_ATELIER_POLL_RETRY_DELAY
...
printf '<h1>near</h1>\n' > "$NEAR_ART"
near_id=$("$ROOT/bin/fm-procevent-atelier.sh" source-id "$NEAR_ART")
PE_TRACKED+=("$HNEAR|$near_id")
ATELIER_COUNT="$TMP_ROOT/near-count"; ATELIER_SCRIPT="near-interrupt feedback"
PATH="$ATELIER_SCRIPTED_BIN:$PATH" FM_HOME="$HNEAR" FM_ATELIER_POLL_RETRY_DELAY=0 \
  "$ROOT/bin/fm-procevent-atelier.sh" arm "$NEAR_ART" >/dev/null
PATH="$ATELIER_SCRIPTED_BIN:$PATH" FM_HOME="$HNEAR" pe "$HNEAR" start "$near_id" >/dev/null
[ "$(cat "$ATELIER_COUNT")" = 1 ] \
  || fail "a near-match interruption was retried instead of surfacing on its first poll"
assert_contains "$(wake_payloads "$HNEAR")" "procevent atelier $near_id 1" \
  "a whitespace variant of the interruption is captured and announced immediately"
PATH="$ATELIER_SCRIPTED_BIN:$PATH" FM_HOME="$HNEAR" \
  "$ROOT/bin/fm-procevent-atelier.sh" retire "$NEAR_ART" >/dev/null
...
invalid_id=$("$ROOT/bin/fm-procevent-atelier.sh" source-id "$INVALID_ART")
for invalid_delay in 61 invalid; do
  invalid_status=0
  invalid_out=$(PATH="$ATELIER_SCRIPTED_BIN:$PATH" FM_HOME="$HINVALID" \
    FM_ATELIER_POLL_RETRY_DELAY="$invalid_delay" \
    "$ROOT/bin/fm-procevent-atelier.sh" arm "$INVALID_ART" 2>&1) || invalid_status=$?
  [ "$invalid_status" -ne 0 ] \
...
ATELIER_COUNT="$TMP_ROOT/quoted-count"; ATELIER_SCRIPT="feedback"
PATH="$ATELIER_SCRIPTED_BIN:$PATH" TMPDIR="$QUOTED_TMPDIR" \
  "$ROOT/bin/fm-procevent-atelier.sh" poll "$NEAR_ART" >/dev/null
quoted_staged=("$QUOTED_TMPDIR"/fm-atelier-poll.*)
[ ! -e "${quoted_staged[0]}" ] \
...
ATELIER_STREAM_READY="$TMP_ROOT/stream-ready"
ATELIER_STREAM_RELEASE="$TMP_ROOT/stream-release"
mkdir -p "$STREAM_TMPDIR"
printf '<h1>stream</h1>\n' > "$STREAM_ART"
stream_id=$("$ROOT/bin/fm-procevent-atelier.sh" source-id "$STREAM_ART")
PE_TRACKED+=("$HSTREAM|$stream_id")
ATELIER_COUNT="$TMP_ROOT/stream-count"; ATELIER_SCRIPT="stream"
PATH="$ATELIER_SCRIPTED_BIN:$PATH" FM_HOME="$HSTREAM" \
  "$ROOT/bin/fm-procevent-atelier.sh" arm "$STREAM_ART" >/dev/null
PATH="$ATELIER_SCRIPTED_BIN:$PATH" TMPDIR="$STREAM_TMPDIR" \
  ATELIER_STREAM_READY="$ATELIER_STREAM_READY" ATELIER_STREAM_RELEASE="$ATELIER_STREAM_RELEASE" \
  FM_PROCEVENT_MAX_OUTPUT_BYTES=100 pe "$HSTREAM" reconcile >/dev/null
wait_for "$ATELIER_STREAM_READY" || fail "streaming poll did not start"
stream_staged=("$STREAM_TMPDIR"/fm-atelier-poll.*)
...
PATH="$ATELIER_SCRIPTED_BIN:$PATH" FM_HOME="$HSTREAM" \
  "$ROOT/bin/fm-procevent-atelier.sh" retire "$STREAM_ART" >/dev/null
pass "Atelier classification staging stays bounded while nonmatches stream"
...
pe_register "$HW" atelier restart-cut-src -- "$BLOCKER" "$TRIGW" "restart cut payload" >/dev/null
pe "$HW" reconcile >/dev/null
sleep 0.5
: > "$TRIGW"
wait_for "$HW/state/.wake-queue" || fail "the restart-cut source published no event"
assert_contains "$(wake_payloads "$HW")" "procevent atelier restart-cut-src 1" \
...
' _ "$ROOT" "$HP")
expected=$(printf '%s\n' \
  'check: procevent atelier ordered-src 1' \
  'check: procevent atelier ordered-src 2' \
  'check: procevent atelier ordered-src 10')
...
pe_register "$HA" atelier shared-src -- "$BLOCKER" "$TRIG2" "shared" >/dev/null
pe_register "$HB" atelier shared-src -- "$BLOCKER" "$TRIG2" "shared" >/dev/null
...
pe_register "$HZ" atelier orphan-src -- "$BLOCKER" "$TRIG4" "orphan" >/dev/null
...
printf '%s\n%s\nstale-token\nstale-identity\n' "$HC" "999999" > "$CLAIM"
chmod 0600 "$CLAIM"
pe_register "$HC" atelier stale-src -- /bin/echo recovered >/dev/null
...
pe_register "$HC_NEW" atelier cross-home-src -- /bin/echo recovered >/dev/null
...
pe_register "$HR" atelier race-src -- "$RACE_BLOCKER" "$RACE_LOG" "$RACE_TRIGGER" >/dev/null
...
pe_register "$HG" atelier orphan-src -- \
...
pe_register "$HG2" atelier dead-gen-src -- "$RACE_BLOCKER" "$DEAD_LOG" "$DEAD_TRIGGER" >/dev/null
...
pe_register "$HJ" atelier torn-src -- "$BLOCKER" "$TORN_TRIGGER" "torn" >/dev/null
...
pe_register "$HK" atelier retire-start-src -- "$START_BLOCKER" "$START_LOG" >/dev/null
...
pe_register "$HI" atelier reused-src -- /bin/true >/dev/null
sleep 60 &
innocent_pid=$!
...
pe_register "$HL" atelier identity-src -- "$BLOCKER" "$IDENTITY_TRIGGER" "identity" >/dev/null
...
pe_register "$HM" atelier sweep-one -- "$BLOCKER" "$SWEEP_TRIGGER_ONE" "sweep one" >/dev/null
pe_register "$HM" atelier sweep-two -- "$BLOCKER" "$SWEEP_TRIGGER_TWO" "sweep two" >/dev/null
...
pe_register "$HN" atelier foreign-src -- "$BLOCKER" "$FOREIGN_TRIGGER" "foreign" >/dev/null
pe_register "$HO" atelier foreign-src -- "$BLOCKER" "$FOREIGN_TRIGGER" "foreign" >/dev/null
...
pe_register "$HU" atelier sweep-uncertain -- "$BLOCKER" "$SWEEP_UNCERTAIN_TRIGGER" "uncertain" >/dev/null
...
pe_register "$HD" atelier argv-src -- "$BLOCKER" "$TRIG3" "one arg with spaces" "second; rm -rf /tmp/nope" >/dev/null
...
newline_status=0
newline_out=$(pe_register "$HD" atelier newline-src -- /bin/echo $'first\nsecond' 2>&1) || newline_status=$?
...
pe_register "$HE" atelier fail-src -- /bin/sh -c 'exit 7' >/dev/null
...
pe_register "$HF" atelier big-src -- /bin/sh -c 'printf "x%.0s" $(seq 1 5000)' >/dev/null
...
SH
chmod +x "$NOISY"
pe_register "$HG" atelier noisy-src -- "$NOISY" "$NOISY_PID" >/dev/null
...
pe_register "$HBAD" atelier bad-limit -- /bin/true >/dev/null
bad_limit_status=0
...
# --- the Atelier adapter uses the published poll shape -----------------------
ART="$TMP_ROOT/artifact.html"
printf '<h1>fixture</h1>\n' > "$ART"
sid=$(FM_HOME="$TMP_ROOT/hg" "$ROOT/bin/fm-procevent-atelier.sh" source-id "$ART")
case "$sid" in atelier-*) : ;; *) fail "adapter source id has an unexpected shape: $sid" ;; esac
sid2=$(FM_HOME="$TMP_ROOT/hg" "$ROOT/bin/fm-procevent-atelier.sh" source-id "$ART")
[ "$sid" = "$sid2" ] || fail "adapter source id is not stable"
ART_ALIAS="$TMP_ROOT/artifact-alias.html"
ln -s "$ART" "$ART_ALIAS"
sid3=$(FM_HOME="$TMP_ROOT/hg" "$ROOT/bin/fm-procevent-atelier.sh" source-id "$ART_ALIAS")
...
newline_artifact_out=$("$ROOT/bin/fm-procevent-atelier.sh" source-id "$ART_NEWLINE" 2>&1) || newline_artifact_status=$?
[ "$newline_artifact_status" -ne 0 ] || fail "Atelier source identity accepted an artifact path ending in a newline"
assert_contains "$newline_artifact_out" "cannot contain newlines" "Atelier rejects newline paths before canonicalization"
...
CLS="$TMP_ROOT/cls"
printf 'session:\n  file: /a.html\n  status: feedback\nprompts[1]{uid}:\n  p1\n' > "$CLS"
out=$("$ROOT/bin/fm-procevent-atelier.sh" classify "$CLS")
assert_contains "$out" feedback "the adapter reads the indented session status"
printf 'session:\n  file: /a.html\n  status: feedback\nprompts[1]{text}:\n  No active Atelier Editor session; code: NOT_FOUND\n' > "$CLS"
assert_contains "$("$ROOT/bin/fm-procevent-atelier.sh" classify "$CLS")" feedback "prompt text cannot override a valid session status"
printf 'session:\n  file: /a.html\n  status: ended\n' > "$CLS"
assert_contains "$("$ROOT/bin/fm-procevent-atelier.sh" classify "$CLS")" ended "an ended session classifies as ended"
printf 'error: No active Atelier Editor session for this file\ncode: NOT_FOUND\n' > "$CLS"
assert_contains "$("$ROOT/bin/fm-procevent-atelier.sh" classify "$CLS")" missing "an explicit missing session classifies as missing"
printf 'garbage that is not a session block\n' > "$CLS"
assert_contains "$("$ROOT/bin/fm-procevent-atelier.sh" classify "$CLS")" unknown "malformed output classifies as unknown rather than a lifecycle state"
pass "the adapter classifies published poll output safely"

# The adapter, not the runner, decides which results end a Atelier source. A
...
printf 'session:\n  file: /a.html\n  status: feedback\n  session_ended: true\n  ended_by: user\n' > "$TRM"
assert_contains "$("$ROOT/bin/fm-procevent-atelier.sh" classify "$TRM")" feedback \
  "a final feedback delivery still classifies as feedback for the handler"
"$ROOT/bin/fm-procevent-atelier.sh" terminal "$TRM" \
...
printf 'session:\n  file: /a.html\n  status: ended\n  ended_by: user\n' > "$TRM"
"$ROOT/bin/fm-procevent-atelier.sh" terminal "$TRM" || fail "an ended session was not reported terminal"
printf 'error: No active Atelier Editor session for this file\ncode: NOT_FOUND\n' > "$TRM"
"$ROOT/bin/fm-procevent-atelier.sh" terminal "$TRM" || fail "a missing session was not reported terminal"
printf 'session:\n  file: /a.html\n  status: waiting\n' > "$TRM"
"$ROOT/bin/fm-procevent-atelier.sh" terminal "$TRM" && fail "a waiting session was reported terminal"
printf 'garbage that is not a session block\n' > "$TRM"
"$ROOT/bin/fm-procevent-atelier.sh" terminal "$TRM" && fail "an unreadable result was reported terminal"
printf 'session:\n  file: /a.html\n  status: feedback\nfeedback[1]{text}:\n  session_ended: true\n' > "$TRM"
"$ROOT/bin/fm-procevent-atelier.sh" terminal "$TRM" \
  && fail "prompt payload text was read as a session-level terminal marker"
pass "the adapter owns which Atelier results end a source, and payload text cannot forge one"

# The adapter, not the runner, decides which Atelier results are routine no-ops
...
silent_says() {  # <expected: yes|no> <description>
  if "$ROOT/bin/fm-procevent-atelier.sh" silent "$SIL" >/dev/null 2>&1; then
...
pass "the adapter owns which Atelier results are silent, and fails closed on everything else"
...
read_out() { "$ROOT/bin/fm-procevent-atelier.sh" read "$READ"; }
cat > "$READ" <<'EOF'
session:
  file: /review.html
  status: feedback
...
# Real Atelier payload shapes, not the prompt==text test-fixture echo:
...
adapter_help=$("$ROOT/bin/fm-procevent-atelier.sh" --help 2>&1 || true)
