...
case " $* " in
  *" -A "*)
    printf '%s %s %s %s\n' "$FM_TEST_ROOT_PID" 100 bash bash
    printf '%s\n' '100 200 /test/Pi.app/bin/pi Pi'
    printf '%s\n' '200 1 /opt/test/bin/pi-signed pi-signed --model test/model'
    exit 0
    ;;
esac
field= pid=
...
SH
  ...
  got=$(PATH="$fakebin:$BASE_PATH" FM_PROC_ROOT_OVERRIDE="$dir/no-proc" bash -c \
    'export FM_TEST_ROOT_PID=$$; . "$0/bin/fm-session-lock-lib.sh"; fm_harness_ancestry_pid' "$ROOT")
