...
test_stale_terminal_status_overridden_by_active_run() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "watcher did not escalate an overridden stale terminal status past the long-call allowance"
...
test_nonterminal_stale_provably_working_absorbed_then_escalated() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "watcher did not escalate a stopped crew whose stale timer passed the threshold"
  grep -F "stale: $window" "$out" >/dev/null || fail "escalation did not print a stale wake"
  grep -F "possible wedge" "$out" >/dev/null || fail "escalation did not flag a possible wedge"
...
test_paused_authoritative_working_preserves_wedge_timer() {
  ...
  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 || fail "authoritative working state did not wedge-escalate past the long-call allowance"
...
test_in_contract_pipeline_call_is_not_wedge_escalated() {
  ...
  while [ "$n" -le 2 ]; do
    ...
    reap "$pid"
    ack_stopped_cycle "$state" || fail "could not acknowledge the intentional in-contract round $n stop"
    n=$((n + 1))
  done
...
# The working deferral must not be allowed to extend this bound. A busy pane
# reports `working · pane` to fm-crew-state.sh, which is exactly what the
# deferral's re-verification reads - so deferring here would re-affirm the one
# signal BUSY_TURN_MAX_SECS exists to stop trusting, and a hung foreground call
# would stay hidden for the bound plus the whole allowance.
test_busy_turn_age_escalation_is_exempt_from_working_deferral() {
  local dir state fakebin out capture_file window key pane_hash sig pid
  dir=$(make_case busy-turn-age-nodefer); state="$dir/state"; fakebin="$dir/fakebin"
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:fm-busy-nodefer"
  printf 'Working...' > "$capture_file"
  printf 'window=%s\nkind=ship\nharness=pi\n' "$window" > "$state/busy-nodefer.meta"
  record_pi_busy "$state" busy-nodefer
  printf 'working: setup complete\n' > "$state/busy-nodefer.status"
  sig=$(seen_sig "$state/busy-nodefer.status"); printf '%s' "$sig" > "$state/.seen-busy-nodefer_status"
  key=$(printf '%s' "$window" | tr ':/.' '___')
  pane_hash=$(hash_text "Working...")
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  touch -t 200001010000 "$state/busy-nodefer.meta"
  echo $(( $(date +%s) - 500 )) > "$state/.stale-since-$key"
  # The crew reads as provably working, well inside the deferral allowance.
  export FM_FAKE_CREW_STATE='state: working · source: pane · harness busy'

  PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
    FM_STATE_OVERRIDE="$state" FM_CREW_STATE_BIN="$fakebin/fm-crew-state.sh" \
    FM_BUSY_TURN_MAX_SECS=1 FM_STALE_ESCALATE_SECS=240 FM_WEDGE_WORKING_ESCALATE_SECS=999999 \
    FM_POLL=1 FM_SIGNAL_GRACE=1 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 40 || fail "the working deferral suppressed a busy pane past its completed-turn bound: $(cat "$out")"
  grep -F "possible wedge" "$out" >/dev/null || fail "busy turn-age escalation did not flag a possible wedge"
  [ ! -e "$state/.wedge-verified-$key" ] || fail "the busy turn-age path recorded a deferral verification marker"
  unset FM_FAKE_CREW_STATE
  pass "a busy pane past its completed-turn bound escalates even while the crew still reads as provably working"
}
...
test_busy_declared_pause_is_rechecked_not_wedge_escalated() {
  ...
  printf 'Working... (7200.4s) atelier-axi poll' > "$capture_file"
  printf 'window=%s\nkind=scout\nharness=pi\n' "$window" > "$state/review-scout.meta"
  record_pi_busy "$state" review-scout
  printf 'paused: hosting the Atelier review, awaiting captain feedback\n' > "$statusf"
...
case "${1:-}" in
  ...
  capture-pane)
    ...
    printf 'Working... (%d.%ds) atelier-axi poll' "$(( 7200 + n ))" "$(( n % 10 ))"
    exit 0 ;;
  display-message)
    case "$*" in
