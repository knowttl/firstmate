...
test_arm_propagates_immediate_parked_wake_before_confirmation() {
  ...
  grep -F "parked: task (awaiting_agent observed" "$armout" >/dev/null || fail "arm did not propagate the immediate parked wake"
...
test_arm_propagates_immediate_possible_wedge_before_confirmation() {
  local dir state fakebin armout drain_out rc
  dir=$(make_case arm-immediate-possible-wedge)
  state="$dir/state"
  fakebin="$dir/fakebin"
  armout="$dir/arm.out"
  drain_out="$dir/drain.out"
  ...
  rc=0
  PATH="$fakebin:$PATH" FM_STATE_OVERRIDE="$state" FM_CREW_STATE_BIN="$fakebin/fm-crew-state.sh" \
    FM_GUARD_GRACE=0 FM_PARKED_SCAN_INTERVAL=0 FM_STALE_ESCALATE_SECS=240 FM_POLL=5 \
    FM_SIGNAL_GRACE=1 FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" || rc=$?
  unset FM_FAKE_CREW_STATE
  [ "$rc" -eq 0 ] || fail "arm returned non-zero for an immediate possible-wedge wake (status $rc): $(cat "$armout")"
  grep -F "possible-wedge: task (state: possible-wedge" "$armout" >/dev/null \
    || fail "arm did not propagate the immediate possible-wedge wake"
  ! grep -qF 'watcher: FAILED' "$armout" || fail "arm printed FAILED after a valid possible-wedge wake"
  FM_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after immediate possible-wedge wake failed"
  grep "$(printf '\tpossible-wedge\t')" "$drain_out" >/dev/null \
    || fail "immediate possible-wedge wake was not queued"
  pass "arm propagates an immediate possible-wedge wake before confirmation"
}
